/**
 * Parser LeParking - Extraction des annonces depuis HTML
 */

import type { ListingResponse } from '@/lib/types'
import { createRouteLogger } from '@/lib/logger'

const log = createRouteLogger('leparking-parser')

export interface LeParkingRawListing {
  title: string
  price: number | null
  year: number | null
  mileage: number | null
...
/**
 * Parse les annonces depuis le HTML LeParking
 */
export function parseLeParkingHtml(
  html: string,
  brand: string,
  model: string,
  maxPrice: number
): LeParkingRawListing[] {
  const listings: LeParkingRawListing[] = []

  try {
    if (html.length < 1000) {
      log.warn('HTML trop court pour LeParking', { htmlLength: html.length })
      return []
    }

    // LeParking utilise souvent des structures avec class="listing" ou data-*
    const listingMatches = html.match(/<div[^>]*class=["'][^"']*listing[^"']*["'][^>]*>[sS]*?<\/div>/g) || []
    const cardMatches = html.match(/<div[^>]*class=["'][^"']*(?:card|item|annonce|vehicule)[^"']*["'][^>]*>[sS]*?<\/div>/g) || []
    ...
    const allMatches = [...listingMatches, ...cardMatches, ...articleMatches]

    log.debug('LeParking parser', {
      listingsFound: listingMatches.length,
    ...
    if (listings.length === 0 && html.length > 50000) {
      ...
      if (links.length > 0) {
        log.debug('LeParking: extraction par liens', { linksCount: links.length })
        for (const linkPath of links) {
          const url = linkPath.startsWith('http') 
            ? linkPath 
            : `https://www.leparking.fr${linkPath}`
    ...
    log.error('Erreur parsing LeParking', {
      error: error instanceof Error ? error.message : String(error),
    })
  }

  log.info('LeParking parser terminé', {
...
function extractListingFromHtml(
  ...
  const url = urlPath.startsWith('http') ? urlPath : `https://www.leparking.fr${urlPath}`
  ...
  const imageUrl = imageMatch ? (imageMatch[1].startsWith('http') ? imageMatch[1] : `https://www.leparking.fr${imageMatch[1]}`) : null
...
export function convertLeParkingToListingResponse(
  raw: LeParkingRawListing,
  index: number
): ListingResponse {
  const id = `leparking_${Date.now()}_${index}`
